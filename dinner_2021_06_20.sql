...
CREATE TABLE `r_orderproduct_deliver` (
  ...
  KEY `fk_dlver` (`delivererid`),
  CONSTRAINT `fk_dlver` FOREIGN KEY (`delivererid`) REFERENCES `dlv_user` (`id`),
  CONSTRAINT `fk_order_product` FOREIGN KEY (`orderid`, `productid`) REFERENCES `t_order_product` (`orderid`, `productid`)
...
CREATE TABLE `r_orderproduct_supplier` (
  ...
  CONSTRAINT `fk_orderproduct` FOREIGN KEY (`orderid`, `productid`) REFERENCES `t_order_product` (`orderid`, `productid`),
  CONSTRAINT `fk_supplier` FOREIGN KEY (`supplierid`) REFERENCES `sp_user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='订单中的商品的供货商';

/*Table structure for table `r_product_company` */

DROP TABLE IF EXISTS `r_product_company`;

CREATE TABLE `r_product_company` (
  `productid` int NOT NULL COMMENT '商品id',
  `companyid` int NOT NULL COMMENT '公司id',
  `start_date` date NOT NULL COMMENT '起始日期',
  `end_date` date NOT NULL COMMENT '结束日期',
...
CREATE TABLE `t_product` (
  ...
  `categoryid` int NOT NULL DEFAULT '0' COMMENT '商品分类',
  `price` decimal(16,2) NOT NULL DEFAULT '0.00' COMMENT '价格',
  `sales` int NOT NULL DEFAULT '0' COMMENT '销量',
  `type` int NOT NULL COMMENT '是否为餐饮 1是 0不是',
  `img` varchar(256) DEFAULT NULL COMMENT '商品图片',
  `crtime` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `ix_name` (`name`),
  KEY `category` (`categoryid`),
  CONSTRAINT `t_product_ibfk_1` FOREIGN KEY (`categoryid`) REFERENCES `t_category` (`id`)
