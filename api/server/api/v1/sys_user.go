...
// 登录以后签发jwt
func tokenNext(c *gin.Context, user model.TUser) {
	...
	if err, jwtStr := service.GetRedisJWT(user.Code); err == redis.Nil {
		if err := service.SetRedisJWT(token, user.Code); err != nil {
...
// @Summary 用户注册账号
// @Produce  application/json
// @Param data body model.TUser true "用户名, 昵称, 密码, 角色ID"
...
func Register(c *gin.Context) {
	...
	user := &model.TUser{Code: R.Code, Nick: R.Nick}
...
//		return
//	}
//	U := &model.TUser{Username: user.Username, Password: user.Password}
...
// @accept application/json
// @Produce application/json
// @Param data body model.TUser true "ID, 用户名, 昵称, 头像链接"
